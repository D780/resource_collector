...
class KisssubScraper extends BaseScraper {
  // 获取番剧列表
  async getAnimeList() {
    ...
    // 解析番剧列表
    $('.list-content .list-item').each((index, element) => {
      const titleElement = $(element).find('.item-title a');
      const title = titleElement.text().trim();
      const link = titleElement.attr('href');
      
      // 提取站点ID
      const siteId = link.match(/\/(\d+)\.html$/)[1];
  ...
  async getAnimeDetail(anime) {
    ...
    // 提取磁力链接
    const magnetLinks = this.extractMagnetLinks($, '.magnet-links a');
